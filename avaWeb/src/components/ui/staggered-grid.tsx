...
export interface StaggeredGridProps {
    /** Number of grid tiles to render (default 21). */
    tileCount?: number
    /** Technologies cycled through the grid tiles. */
    techStack: TechTile[]
...
export function StaggeredGrid({
    ...
    useEffect(() => {
        ...
        // Wait for background images to load before running entrance timelines.
        imagesLoaded(document.querySelectorAll('.grid__item-img'), { background: true }, handleLoad)

        // Fallback: if there are no images to wait on, fire anyway.
    ...
    // Prepare grid items: fill up to the end of Row 3 (default 21 slots)
    // This perfectly balances the 3rd row with 2 cards on each side of the bento container.
    const mixedGridItems: ('TILE' | 'BENTO_GROUP')[] = Array.from({ length: tileCount }, () => 'TILE');
            ...
            <section className="grid place-items-center w-full relative">
                <div ref={gridFullRef} className="grid--full relative w-full my-[10vh] h-auto aspect-[1.1] max-w-none p-4 grid gap-4 grid-cols-7 grid-rows-5">
                    ...
                    {mixedGridItems.map((item, i) => {
                        ...
                        if (item === 'TILE') {
                            const tech = techStack.length ? techStack[i % techStack.length] : null;

                            return (
                                <figure key={`tech-${i}`} data-col={i % 7} className="grid__item m-0 relative z-10 [perspective:800px] will-change-[transform,opacity] group cursor-pointer">
                                    <div className="grid__item-img w-full h-full [backface-visibility:hidden] will-change-transform rounded-xl overflow-hidden shadow-sm border border-zinc-200 dark:border-white/10 bg-zinc-100 dark:bg-white/[0.03] flex items-center justify-center transition-all duration-500 ease-out group-hover:scale-105 group-hover:shadow-xl group-hover:border-transparent">
                                        ...
                                        {/* Content Container */}
                                        <div className="relative z-10 flex flex-col items-center justify-center gap-2 px-1 text-center">
                                            {/* Tech icon */}
                                            <span className="text-2xl text-zinc-400 dark:text-zinc-400 transition-all duration-300 group-hover:text-white group-hover:scale-110">
                                                {tech?.icon}
                                            </span>

                                            {/* Name reveal on hover */}
                                            <div className="opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300 delay-75">
                                                <span className="block font-mono text-[9px] font-medium uppercase tracking-wider text-white">{tech?.name}</span>
                                            </div>
                                        </div>
                                    </div>
                                </figure>
