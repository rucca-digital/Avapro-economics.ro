...
export interface TwistingRibbonProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  twistCycles?: number;
  /** Half-thickness of the ribbon band in px (default 14). Larger = more
   *  screen coverage, so it reads as a flowing background rather than a strip. */
  ribbonWidth?: number;
...
export function TwistingRibbon({
  ...
  useEffect(() => {
    ...
    // Cap device pixel ratio — Android phones report 2.5–4x, which makes the
    // canvas enormous and the per-frame draw far too expensive.
    ...
    // ── Configuration ───────────────────────────────────────────────────
    const RIBBON_HALF_W = ribbonWidth;
    ...
    // Only animate while the ribbon is actually on screen — keeps the canvas
    // from burning battery/CPU when scrolled past (critical on mobile).
  ...
  }, [segments, waveSpeed, waveAmplitude, twistCycles, ribbonWidth, lightColors, darkColors]);
